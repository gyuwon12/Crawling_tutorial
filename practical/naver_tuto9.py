# 네이버 본문 크롤링 결과를 엑셀에 저장하기 + 마지먁 페이지 확인 기능
...
for i in range(input_page_num): # 10페이지동안
    page_number = i + 1 # 1, 2, 3, ...
    response = requests.get(f"https://search.naver.com/search.naver?where=news&sm=tab_jum&query={input_query}&start={10*i+1}") # &이 토큰 구분자라 이 토큰이 너무나도 중요함
    html = response.text
    ...
    for link in links:
        if len(link.select("a")) >= 2:
            ...
            if "https://entertain.naver.com" in response.url: 
                title = soup_sub.select_one(".end_tit").text
                article_content = soup_sub.select_one("#articeBody").text.strip()
            # 2. 스포츠 뉴스
            elif "https://sports.news.naver.com" in response.url: 
                title = soup_sub.select_one("h4.title").text
                article_content = soup_sub.select_one("#newsEndContents")
            ...
            # 나머지는 일반화
                ...
                title = soup_sub.select_one("#title_area").text
                article_content = soup_sub.select_one("#newsct_article").text.strip()
    ...
    # 마지막 페이지 여부 확인하기
    is_last_page = soup.select_one("a.btn_next").attrs['aria-disabled'] # 문자열로 담김
    if is_last_page == 'true': # 마지막 페이지 라는 것임
        print(f"마지막 페이지 입니다. {page_number} 페이지")
        break
