...
for i in range(input_page_num): # 10페이지동안
    ...
    for link in links:
        if len(link.select("a")) == 2:
            ...
            #print(f"{title}:\n {article_content}")
            document.add_heading(f"기사 제목: {title}", level=0)
            document.add_paragraph(f"링크: {response.url}")
            document.add_paragraph(f"{article_content}")
            time.sleep(0.2) # 안전성을 위하여

document.save(f"네이버 뉴스 기사 본문 크롤링 키워드: {input_query}.docx")